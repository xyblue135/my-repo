...
# 复制1liucheng/output.txt到2liucheng/95对比/input.txt
source_output_path = os.path.join('1liucheng', 'output.txt')
target_input_path = os.path.join('2liucheng', '95对比', 'input.txt')
...
# 在2liucheng/95对比目录下执行python3 start.py
try:
    subprocess.run(['python3', start_script_name], check=True, cwd=os.path.join('2liucheng', '95对比'))
    print("2liucheng/95对比/start.py 已成功执行。")
except subprocess.CalledProcessError:
    print("执行 2liucheng/95对比/start.py 时发生错误。")
