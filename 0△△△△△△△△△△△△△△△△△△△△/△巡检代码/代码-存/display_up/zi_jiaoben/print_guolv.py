...
# 将结果写入上级目录的upload文件夹中
with open('../upload/success.txt', 'w', encoding='utf-8') as success_file:
    success_file.writelines(success_lines)

with open('../upload/failed.txt', 'w', encoding='utf-8') as failed_file:
    failed_file.writelines(failed_lines)

print("过滤完成，结果已保存upload/success.txt 和 upload/failed.txt 文件中。")
